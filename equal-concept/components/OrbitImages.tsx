...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }: OrbitItemProps) {
  ...
  // Calculate the percentage value (0 to 100)
  const offsetPercent = useTransform(progress, (p: number) => {
    return (((p + itemOffset) % 100) + 100) % 100;
  });

  // Calculate offsetDistance as string
  const offsetDistance = useTransform(offsetPercent, (op: number) => `${op}%`);

  // 3D Depth layering:
  // 0% to 50% is the bottom half (foreground): scale is 1.15, opacity is 1.0, zIndex is 25.
  // 50% to 100% is the top half (background): scale is 0.8, opacity is 0.5, zIndex is 5.
  const scale = useTransform(offsetPercent, [0, 25, 50, 75, 100], [1.15, 1.15, 0.8, 0.8, 1.15]);
  const opacity = useTransform(offsetPercent, [0, 25, 50, 75, 100], [1.0, 1.0, 0.5, 0.5, 1.0]);
  const zIndex = useTransform(offsetPercent, (op: number) => {
    return op < 50 ? 25 : 5;
  });
