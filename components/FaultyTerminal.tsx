...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    const reduced = mediaMatches("(prefers-reduced-motion: reduce)");
    const coarse = mediaMatches("(pointer: coarse)");
    const resolvedPause = pause || reduced;
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uScanlineIntensity: { value: coarse ? scanlineIntensity * 0.55 : scanlineIntensity },
        uGlitchAmount: { value: coarse ? 0.65 : glitchAmount },
        uFlickerAmount: { value: coarse ? 0.2 : flickerAmount },
        ...
        uUseMouse: { value: resolvedMouse ? 1 : 0 },
        uPageLoadProgress: { value: pageLoadAnimation && !reduced ? 0 : 1 },
        uUsePageLoadAnimation: { value: pageLoadAnimation && !reduced ? 1 : 0 },
        uBrightness: { value: coarse ? brightness * 0.72 : brightness },
      },
    });
    ...
    const update = (t: number) => {
      ...
      if (!resolvedPause) {
        const elapsed = (t * 0.001 + timeOffsetRef.current) * (coarse ? timeScale * 0.45 : timeScale);
      ...
      if (pageLoadAnimation && !reduced && loadAnimationStartRef.current > 0) {
        const progress = Math.min((t - loadAnimationStartRef.current) / 1800, 1);
    ...
    rafRef.current = requestAnimationFrame(update);
    if (resolvedMouse) ctn.addEventListener("mousemove", handleMouseMove);

    return () => {
      cancelAnimationFrame(rafRef.current);
      resizeObserver.disconnect();
      if (resolvedMouse) ctn.removeEventListener("mousemove", handleMouseMove);
  ...
  return (
    <div
      ref={containerRef}
      className={`hero-shader-layer relative h-full w-full overflow-hidden ${className}`}
      style={style}
      aria-hidden="true"
      {...rest}
    />
